...
if (! function_exists('mca')) {
    ...
    function mca()
    {
        $mca = explode('\\', request()->route()->action['controller']);

        $moduleName = $mca[count($mca)-2] == 'Controllers' ? null : $mca[count($mca)-2];

        $func = explode('@', $mca[count($mca) - 1]);

        $controllerName = str_replace('Controller', '', $func[0]);
        $actionName = $func[1];

        return ['module' => $moduleName, 'controller' => $controllerName, 'action' => $actionName];
    }
}

if(! function_exists('arrayColumns')) {
    function arrayColumns($arr, $key)
    {
        $a = array();
...
function getHtml5Img($base64_image_content) {
