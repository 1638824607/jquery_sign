...
<head>
    <title>{{$chapterTitle}}-{{$bookName}}-入香阁</title>
    <meta charset="utf-8" />
    <meta http-equiv="Cache-Control" content="no-transform" />
    <meta http-equiv="Cache-Control" content="no-siteapp" />
    <meta name="Baiduspider" content="noarchive" />
    <meta name="keywords" content="{{$chapterTitle}},{{$bookName}}" />
    <meta name="description" content="{{$bookName}}最新章节{{$chapterTitle}}免费阅读" />
...
<div class="newwap read ">
    <div class="readhd clearfix">
        <div class="clearfix top">
            <div class="container clearfix">
                <span class="fl left"><a href="{{url('poly/detail/'.$bookId)}}" class="backgo" title="入香阁">入香阁</a></span>
    ...
    <div class="readcont clearfix ">
        <div class="container clearfix">
            <div class="bigfontx" data-rel="{{$chapterId}}">
                <div class="model0">
                    <div class="mdxx">
                        <h1 class="mdxx-title">{{$chapterTitle}}</h1>
                    </div>
                    <div class="con_tent">
    ...
    <div class="readtool updown">
        <div class="container">
            <div class="first">
                <div class="moretool">
                    <a href="{{url('poly/content/'.(max(0, $chapterId-1)).'/'.$bookId)}}" class="m1 fl">上一章</a>
                    <a href="{{url('poly/dir/'.$bookId)}}" class="m2 fl">目录</a>
                    <a href="{{url('poly/content/'.($chapterId+1).'/'.$bookId)}}" class="m3 fl">下一章</a>
                </div>
            </div>
        </div>
    </div>
        ...
        if(isGo){
            ...
            if(DocH -(STH+WinH) <= 3){
                ...
                if(chapter_id>=0){
                    ...
                    $.ajax({
                        type: "POST",
                        url: "{{url('poly/content')}}/" + (parseInt(chapter_id)+1) + '/' + book_id,
                        dataType:'json',
                        success: function(data){
                            var html = '';
                            if(data.status == 2)
                            {
                                setTimeout(function(){
                                    ...
                                    html = '<div class="model0"><div class="mdxx"><h1 class="mdxx-title">'+data.data.chapterTitle+'</h1>' +
                                        '</div><div class="con_tent">    　　'+data.data.chapterContent+'</div></div>';

                                    $('.bigfontx').append(html);
                                    $('.bigfontx').attr('data-rel',data.data.chapterId);
                                    $('.addload').hide();

                                    history.replaceState('', null, '/poly/content/'+data.data.chapterId+'/'+data.data.bookId+'');
